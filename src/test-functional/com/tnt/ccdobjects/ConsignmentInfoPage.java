...
public class ConsignmentInfoPage {
	...
	By dimension=By.xpath("(//input[@name='[object Object]'])[1]");
	By consignmentNote=By.xpath("//label//input[@name='Consignment Note']/following::span[@class='slds-checkbox_faux'][1]");
	...
	By totalVolumeExceedMsg=By.xpath("//div[contains(text(),'Total Volume exceeds restrictions for selected service')]");
	By totalWeigthExceedMsg=By.xpath("//div[contains(text(),'You can only ship boxes up to 70 kg in weight.')]");
	By quantityExceedMsg=By.xpath("//div[contains(text(),'The maximum number of packages in a single shipment is 99.')]");
	By quantityMinErrorMsg=By.xpath("//div[contains(text(),'The minimum number of packages in a single shipment is 1.')]");
	...
	public String getDimension() {
		...
		return dim.getAttribute("value");
	}
	
	public void setDimension(String dimMeasurement,int index) {
		WebElement dim=uiTestHelper.waitForObject(dimension);
		uiTestHelper.clickJS(dim);
		WebElement dim1=uiTestHelper.waitForObject(By.xpath("//lightning-base-combobox-item[@data-value='"+dimMeasurement+"']["+index+"]"));
		dim1.click();
	}
