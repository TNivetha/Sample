...
public class CCD_Connectivity extends Driver {
	...
	public CCD_Connectivity() {
		ttp_obj = new TrackAndTracePage(driver);
		cp_obj = new ConsignmentPage(driver);
	...
	public void CloseTab() {
		...
		UiTestHelper uiTestHelper = new UiTestHelper();
		try {
			List<WebElement> si = driver.findElements(By.xpath(
	...
	public int cmod_CloseTab(){
		...
		List<WebElement> si = driver.findElements(By.xpath("(//button[contains(@title,'Close')])"));
	...
	public void casetable() {
		try {
			...
			driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
			WebElement recentlyViewed = driver.findElement(By.xpath(
					"//span[text()='Recently Viewed' and @class='triggerLinkText selectedListView uiOutputText']"));
			((JavascriptExecutor) driver).executeScript("arguments[0].click();", recentlyViewed);

			driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
			List<WebElement> myOpenCases = driver.findElements(By.xpath("//input[@role='combobox']"));

			ob.click(myOpenCases.get(1)).perform();
			driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);

			driver.findElement(By.xpath("//input[@role='combobox']")).sendKeys("1 - My Open Cases");

			wait.until(ExpectedConditions
					.presenceOfElementLocated(By.xpath("//span[contains(text(),'1 - My Open Cases')]")));
			WebElement opc = driver.findElement(By.xpath("//span[contains(text(),'1 - My Open Cases')]"));
			driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
	...
	public String TrackandTraceCCDSearch(){
		...
		try {
			HomePage homePage = new HomePage(driver);
			TrackAndTracePage trackandtracePage = new TrackAndTracePage(driver);
	...
	public void deliveryAreaInfoService_in_Consignment_Details(){
		try {
			...
			driver.findElement(By.xpath("//button[@name='closedeliveryarea']")).click();
		}
	...
	public void trackandTrace_PartialConsignmentSearch(){
		try {
			...
			WebElement myTable = driver.findElement(ttp_obj.cognTbl());
			...
			for (int i = 1; i <= row_count; i++) {
				String a = driver
						.findElement(By.xpath(table + "tr[" + i + "]/td[1]//span/div/lightning-base-formatted-text"))
						.getText();
				...
				if (a.equals(PartialCon.trim())) {
					driver.findElement(By.xpath(table + "tr[" + i + "]/th//div[1]//button/lightning-primitive-icon"))
							.click();
	...
	public void consignment_RatingAndInvoicing_from_CasePage(){
		try {
			...
			RatingInvoicePage ripage = new RatingInvoicePage(driver);
			HomePage homepage = new HomePage(driver);
			CustomerAccountPage capage = new CustomerAccountPage(driver);
			...
			/*
			 * WebElement RatingTable = driver.findElement(By.xpath(
	...
	public void TTSearch_9DigitCon(){
		try {
			...
			ttp_obj.clickTrackandTraceSearch();
			if (driver.findElement(cp_obj.txttopCongnNum()).isDisplayed()) {
	...
	public void TTSearch_6DigitsCon(){
		try {
			...
			if (driver.findElements(ttp_obj.cognTbl()).isEmpty()) {
				Fail_Message("Consignment cannot be searched successfully " + Partial);

			} else {
				List<WebElement> CustRef = driver.findElements(ttp_obj.listCognTbl());
				if (CustRef.size() > 0) {
	...
	public void TTSearch_CustReference(){
		try {
			...
			if (driver.findElements(ttp_obj.cognTbl()).isEmpty()) {
				...
				List<WebElement> CustRef = driver.findElements(ttp_obj.cognTbl());
				if (CustRef.size() > 0) {
					System.out
	...
	public void TTSearch_SendAcctAndCountryDelDate(){
		try {
			...
			ttp_obj.clickTrackandTraceSearch();
			if (driver.findElements(ttp_obj.cognTbl()).isEmpty()) {
				if (driver.findElements(ttp_obj.consgnOverview()).isEmpty()) {
					Fail_Message(
	...
	public void TTSearch_ColDate(){
		try {
			...
			WebElement dateWidget = driver.findElement(ttp_obj.calTable());
			...
			WebElement dateWidget1 = driver.findElement(ttp_obj.calTable());
	...
	public void TrackandTraceDrpDown(){
		try {
			...
			if (driver.findElements(ttp_obj.txtCosgnNum()).size() > 0) {
	...
	public void TrackandTraceNewSearchBtn(){
		try {
			...
			if (driver.findElement(By.xpath("//button[text()='New Search']")).isDisplayed()){
	...
	public void TTSearch_CreateUnallocatedCase(){
		try {
			TrackandTraceDrpDown();
			CreateCasePage ccpage = new CreateCasePage(driver);
			ttp_obj.ipCosgnNum("123456789");
			ttp_obj.clickTrackandTraceSearch();
			Pass_Message("Invalid consignment number error message displayed");
			driver.findElement(By.xpath("//div[text()='ERROR:']/following::lightning-primitive-icon[1]")).click();
	...
	public void TTSearch_ViewConNumAtTop(){
		try {
			...
			/*
			 * String conNum =
			 * driver.findElement(cp_obj.topCosginmentNum(Retrive)).getText(); if
			 * (conNum.isEmpty()) {
	...
	public void TTSearch_ClearBtn(){
		...
		try {
			...
			jse2.executeScript("arguments[0].scrollIntoView();", driver.findElement(ttp_obj.txtconsnEquiry()));
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void TTSearch_Verify_ClearAndNewSearchBtn(){
		try {
			...
			if (driver.findElement(ttp_obj.btnNewSearch()).isDisplayed()) {
				new WebDriverWait(driver, 10).until(ExpectedConditions.elementToBeClickable(ttp_obj.btnNewSearch()));
				Pass_Message("New Search button is working fine");
				ttp_obj.clkNewSearch();
			}
			if (driver.findElement(ttp_obj.btnClear()).isDisplayed()) {
				new WebDriverWait(driver, 10).until(ExpectedConditions.elementToBeClickable(ttp_obj.btnClear()));
	...
	public void TTSearch_ConDtlBtn(){
		try {
			TrackandTraceDrpDown();
			if (driver.findElement(ttp_obj.txtCosgnNum()).isDisplayed()) {
				TrackandTraceCCDSearch();
			}
			if (driver.findElement(cp_obj.overviewCosgn()).isDisplayed()) {
				System.out.println("Consginment History" + driver.findElement(cp_obj.viewbtnHistory()).isEnabled());
				System.out.println("Consginment Pieces" + driver.findElement(cp_obj.viewbtnPieces()).isEnabled());
				System.out.println("Consginment View Case" + driver.findElement(cp_obj.viewcase()).isEnabled());
	 ...
	 * (driver.findElement(By.xpath("//*[contains(text(),'View Case')]")).
	...
	public void TTSearch_InvfromConDtlPage(){
		try {
			...
			driver.switchTo().frame(driver.findElement(By.xpath("//iframe[@title='Invoices']")));
	...
	public void TTSearch_ConHistDtlsPage_VerifySections(){
		try {
			TTSearch_ConHistFrmDropdown();
			if (driver.findElement(cp_obj.statusCosgnHistory()).isDisplayed()
					&& driver.findElement(cp_obj.statusHistoryCosgnHistory()).isDisplayed()) {
				System.out.println("Consigment History for status and status History is enable");
			}
			if (driver.findElement(cp_obj.srcCosgnHistory()).isDisplayed()
					&& driver.findElement(cp_obj.rmkNotesCosgnHistory()).isDisplayed()) {
	...
	public void TTSearch_ConPiecesFromDropdown(){
		try {
			...
			if (driver.findElements(ttp_obj.cognTbl()).isEmpty()) {
				...
				if (driver.findElement(cp_obj.piecesNoConsgn()).isDisplayed()
						&& driver.findElement(cp_obj.pieceStatusCosgn()).isDisplayed())
	...
	public void TTSearch_ConHistFrmDropdown(){
		try {
			...
			if (driver.findElements(ttp_obj.cognTbl()).isEmpty()) {
				...
				if (driver.findElement(cp_obj.dateTimeCosgnHistory()).isDisplayed()
						&& driver.findElement(cp_obj.locatnCosgnHistory()).isDisplayed())
	...
	public void TTSearch_ConHist_StatusCodeDtls(){
		try {
			...
			// driver.findElement(By.xpath("//*[contains(@id,'brandBand')]/div/c-trackntrace-container-comp/c-consignment-history-component/lightning-card/article/div/slot/div[3]/section/div/footer/button")).click();
	...
	public void TTSearch_SearchListColSeq(){
		try {
			...
			List<WebElement> table = driver.findElements(cp_obj.listConsignment());
	...
	public void TTSearch_CreateCaseErrorforPE(){
		try {
			...
			ccpage.clickOrigin();
			driver.findElement(By.xpath("//footer[@class='slds-modal__footer']/button[1]")).click();
	...
	public void TTSearch_Verify_LateststatusCodeatTop(){
		try {
			TTSearch_ConHistFrmDropdown();
			String latestCode = driver.findElement(By.xpath("//table/tbody/tr[1]/td[2]//lightning-base-formatted-text"))
					.getText();
	...
	public void validateConsignmentNoDisplayOnTop(String consignmentNo){
		// String conNum =
		// driver.findElement(cp_obj.topCosginmentNum(consignmentNo)).getText();
	...
	public String TrackandTraceCCD(String Retrive){
		try {
			 ...
			 * TrackAndTracePage(driver); trackandtracePage.clkNewSearch();
			 * trackandtracePage.setConsignmentNumber(Retrive);
			 * 
			 * trackandtracePage.clickTrackandTraceSearch();
			 */
	...
	public void verifyComponentError(){
		...
		try {
			WebElement ele = driver.findElement(componentError);
			if (ele.isDisplayed()) {
	...
	public String getBookingUpdateFailiureMessage() {
		WebElement ele = driver.findElement(bookingFailiure);
		return ele.getText();
	}
	...
	public String getImadeErrorMessage() {
		WebElement ele = driver.findElement(imadeError);
		return ele.getText();
	}
	...
	public void verifyGenralEnquiryScreen() {
		try {
			WebElement ele = driver.findElement(generalEnquiryScreen);
			if (ele.isDisplayed()) {
				WebElement close = driver.findElement(closeGEWindow);
				uiTestHelper.clickJS(close);			
			}
		} catch (Exception e) {
