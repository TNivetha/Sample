...
public class ConsignmentPage {
	...
	public boolean verifyViewCase2() throws Exception {
		try {
			By viewcasebtn = By.xpath("//button[contains(text(),'View Case')]");
			boolean isViewcase = driver.findElement(viewcasebtn).isDisplayed();
			return isViewcase;
	...
	public boolean verifyCreateCase() {
		try {
			By createcasebtn = By.xpath("//button[contains(text(),'Create Case')]");
			boolean isCreatecase = driver.findElement(createcasebtn).isDisplayed();
			return isCreatecase;
		} catch (org.openqa.selenium.NoSuchElementException e) {
			return false;
		}
	...
	public void clickViewCaseIcon() {
		...
		jse.executeScript("arguments[0].scrollLeft += 800", ele);
