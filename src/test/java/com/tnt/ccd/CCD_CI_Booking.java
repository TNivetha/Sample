...
public class CCD_CI_Booking extends Driver {
	...
	public void ci_login(String env) {
		try {
			...
			// Assert.assertEquals(URL, getDriver().getCurrentUrl(), "Actual page URL is not same
			// as expected");
			...
			getDriver().manage().timeouts().pageLoadTimeout(90, TimeUnit.MINUTES);
			HomePage homepage = new HomePage(getDriver());
	...
	public void login_SLUser() {
		try {
			...
			login = new LoginPage(getDriver());
			getDriver().manage().window().maximize();
			Assert.assertEquals(URL, getDriver().getCurrentUrl(), "Actual page URL is not same as expected");
			login.setUsername(Username);
			login.setPassword(Password);
			login.clickLoginBtn();
			HomePage homepage = new HomePage(getDriver());
	...
	public void ci_GS_FirstName() {
		try {
			...
			GlobalSearch globalSearch = new GlobalSearch(getDriver());
	...
	public void CI_GS_Header() {
		try {
			...
			CustomerAccountPage customerAccPage = new CustomerAccountPage(getDriver());
	...
	public void ci_GS_VerifyStream() {
		try {
			...
			getDriver().findElement(By.xpath("//input[contains(@title,'Search')]")).clear();
		}
		ACM_Connectivity.CloseTab();
	}
	...
	public void BK_US603140_GoodsInfo_DefaultCCY() {
		try {
			...
			getDriver().findElement(By.xpath("//input[@name='userCurrency']"));
	...
	public void BK_Book_MultipleItems_Overlimits() {
		try {
			...
			getDriver().findElement(By.xpath("(//label[text()='Length']/following::input[1])[2]")).sendKeys("20", Keys.TAB);

			getDriver().findElement(By.xpath("(//label[text()='Width']/following::input[1])[2]")).sendKeys("90", Keys.TAB);
			getDriver().findElement(By.xpath("(//label[text()='Height']/following::input[1])[2]")).sendKeys("21", Keys.TAB);
			getDriver().findElement(By.xpath("(//label[text()='Weight (kg)']/following::input[1])[2]")).sendKeys("71",
					Keys.TAB);
		...
		try {
			...
			for (j = 1; j < 10; j++) {
				for (i = 1; i < 51; i++) {
					...
					List<WebElement> val = getDriver().findElements(By.xpath("//input[@inputmode='decimal']"));
	...
	public void BK_603160_Book_0Items_Error() {
		try {
			BK_Nav_ConsignmentInformation_page();
			ConsignmentInfoPage coninfopage = new ConsignmentInfoPage(getDriver());
			getDriver().findElement(By.xpath("(//input[@name='quantity'])[1]")).sendKeys("0", Keys.TAB);
			;
	...
	public void BK_AdditionalInfo_Page() {
		...
		addinfopage.setCollectionInstruction("Test instruction for collection getDriver()");
		addinfopage.setDeliveryInstruction("Test instruction for delivery getDriver()");
		uiTestHelper.scrolldown("700");
		// Select From time
	...
	public void BK_US605028a_Update_Collection_Instruction() {
		try {
			...
			try {
				...
				Pass_Message("Collection getDriver() instructions updated successfully");
			} catch (Exception e) {
				e.printStackTrace();
				Fail_Message("Exception: Update to Collection getDriver() Instruction failed");
			}
		} catch (Exception e) {
			e.printStackTrace();
			Fail_Message("Navigation to Consignment Information page failed");
	...
	public void BK_Update_CollDriver_Inst_charsLength() {
		...
		try {
			...
			adtnlinfopage.setCollectionInstruction(
					"Please enter more than eighty chars in collection getDriver() instruction in the Additioal Information Page of Booking");
			int CollDriverInstlen = adtnlinfopage.getCollectionInstruction().length();
			System.out.println(CollDriverInstlen);
			Assert.assertEquals(CollDriverInstlen, 80);
	...
	public void BK_US605028c_Update_DelgetDriver_Inst_Morethan50chars() {
		try {
			BK_BookingPage_SISP();
			BK_GoodsInfoPage();
			BK_ConInfo_Page();
			...
			try {
				...
				Pass_Message("Valid Service is selected");
				uiTestHelper.scrolldown("300");
				adtnlinfopage.setDeliveryInstruction("Please enter more than 50 chars in coll getDriver() inst");
				if (adtnlinfopage.getDeliveryInstruction()
						.equals("Please enter more than 50 chars in delv getDriver() ins")) {
					Pass_Message("User is able to enter only 50 chars in Delivery getDriver() field");
				}
			} catch (Exception e) {
				e.printStackTrace();
				Fail_Message("Exception: 50 chars field length fro Delivery getDriver() instruction field test case failed");
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	// update del getDriver() instructions
	public void BK_US605028d_Update_DelDriver_Instruction() {
		try {
			...
			try {
				...
				Pass_Message("Delivery getDriver() instructions updated successfully");
			} catch (Exception e) {
				e.printStackTrace();
				Fail_Message("Exception: Update to Delivery getDriver() Instruction failed");
			}
		} catch (Exception e) {
			e.printStackTrace();
			Fail_Message("Exception: Update to Delivery getDriver() Instruction failed");
	...
	public void BK_US605036_Validate_AvailableServicesDisplayed() {
		try {
			...
			BK_ConInfo_Page();
			try {
				WebElement valServices = getDriver().findElement(By.xpath("//button[@title='Valid Services']"));
				valServices.click();
				wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(
						"//*[contains(text(),'Services Available')]/following::slot[2]//div[@class='slds-scrollable_y']/table")));
				WebElement valServiceTable1 = getDriver().findElement(By.xpath(
				...
				for (int i = 1; i <= row_count1; i++) {
					WebElement mySelect = getDriver().findElement(By.xpath(
	...
	public void BK_US605048_Verify_ViewSummaryBtn() {
		try {
			...
			uiTestHelper.propagateException();
			uiTestHelper.scrolldown("300");
			adtnlinfopage.setCollectionInstruction("Test instruction for collection getDriver()");
			adtnlinfopage.setDeliveryInstruction("Test instruction for delivery getDriver()");
			uiTestHelper.scrolldown("700");
			// Select From time
			try {
				adtnlinfopage.setFromCollWindowTimeInput("17:30");
	...
	public void BK_GS_RetriveBooking() {
		try {
			...
			getDriver().findElement(bookingsearch).click();
			wait.until(ExpectedConditions
	...
	public void Q_QuoteDetails_Validation() {
		try {
			...
			QuoteDetailPage quoteDetailPage = new QuoteDetailPage(getDriver());
			try {
	...
	public void Q_AdditionalInfoPage() {
		...
		QuoteAdditionalInfoPage quoteAdditionalInfoPage = new QuoteAdditionalInfoPage(getDriver());
		AdditionalInfoPage additionalinfopage = new AdditionalInfoPage(getDriver());
		Assert.assertEquals(additionalinfopage.verifyTitle(), true, "Additional Information Page not Opened");
	...
	public void Q_Verify_BackDatedCollDate() {
		try {
			...
			if (getDriver().findElement(By.xpath("//div[contains(text(),'Value must be')]")).isDisplayed()) {
	...
	public void BK_LineItem_BeforeDelete() {
		try {
			BookingRecordPage bkdetailpage = new BookingRecordPage(getDriver());
	...
	public void BookingDrpDown() {
		try {
			...
			if (getDriver().findElements(By.xpath("(//span[contains(text(),'Bookings')])[3]")).size() > 0) {
	...
	public void BK_BookingAuditPageValidation_EditBooking() {
		try {
			...
			List<WebElement> tablerow = getDriver()
			...
			for (int i = 0; i <= rowCount; i++) {
				...
				if (fieldName.equalsIgnoreCase("Collection Contact Email")) {
					String editedDateTime = getDriver().findElement(
							By.xpath("//h2/span[contains(text(),'Audit History')]/following::table[1]/tbody/tr[" + i
	...
	public void createConsignmentNumber() {
		try {
			BookingRecordPage bkrecordpage = new BookingRecordPage(getDriver());
	...
	public void Q_QuoteSummary() {
		try {
			...
			int row_count = objRow.size();
			for (int i = 1; i <= row_count; i++) {
				String tempQuoteNum1 = getDriver()
				...
				if (quoteNumSumm.contains(tempQuoteNum1)) {
					...
					WebElement chkBox = getDriver().findElement(By.xpath(
	...
	public void BK_DG_VerifyDG_TypesofGoodsandUNNumber() {
		...
		try {
			...
			List<WebElement> DGlist = getDriver().findElements(By.xpath(dgoptions));
			List<WebElement> dgOption = getDriver().findElements(By.xpath(dgoptions + "/span[@class='slds-media__body']"));
	...
	public void bk_bookingSummary_emailLanguage_Validation() {
		try {
			...
			getDriver().get("https://outlook.office365.com");
			WebElement email = getDriver().findElement(By.name("loginfmt"));
	...
	public void Check_Green_Tick_For_Available_Services_For_Quote() {
		try {
			...
			QuoteDetailPage quotedetailspage = new QuoteDetailPage(getDriver());
	...
	public void Q_AdditionalInfoPage_SpecialServices() {
		try {
			try {
				...
				String quote = getDriver().findElement(By
						.xpath("//span[contains(text(),'Quote is created successfully. Quote Reference Number is: ')]"))
						.getText();
				quoteNum = quote.replace("Quote is created successfully. Quote Reference Number is: ", "");
				uiTestHelper.propagateException();
	...
	public String BK_ConInform_Page() {
		...
		uiTestHelper.propagateException();
		getDriver().findElement(By.xpath("(//button[@name='[object Object]'])[2]")).click();
		getDriver().findElement(By.xpath("//lightning-base-combobox-item[@data-value='box']")).click();
		getDriver().findElement(By.xpath("(//button[@name='[object Object]'])[4]")).click();
		getDriver().findElement(By.xpath("(//lightning-base-combobox-item[@data-value='pallet'])[2]")).click();
		for (HashMap.Entry<String, String> entry : hashmap.entrySet()) {
			getDriver().findElement(By.xpath(entry.getKey())).sendKeys(entry.getValue());
		}
		Pass_Message("Quantity , Weight and Dimensions are updated on the Consignment Information Page");

		Float quantity1Weight = Float.parseFloat(getDriver()
				.findElement(By.xpath("(//label[text()='Weight (kg)']/following::input[1])[1]")).getAttribute("value"));
		Float quantity2Weight = Float.parseFloat(getDriver()
	...
	public void q_AdditionalInfoPagePrice() {
		...
		try {
			...
			try {
				 ...
				 * Fail_Message("Quote creation failed"); } String quote= getDriver().findElement(By.
				 * xpath("//span[contains(text(),'Quote is created successfully. Quote Reference Number is: ')]"
				 * )).getText(); quoteNum=
				 ...
				 * getDriver().navigate().refresh(); uiTestHelper.propagateException();
				 */
			} catch (Exception e) {
				e.printStackTrace();
				Fail_Message("Exception: Quote failed");
	...
	public void validate_Services_when_edits_on_additionalInformationPage() {
		...
		Pass_Message("getDriver() Instruction and Collecton Window Updated for the Booking");
		Assert.assertEquals(adtnlinfopage.verifyViewSummary(), true, "Service Needs to be revalidated");
		if (adtnlinfopage.verifyViewSummary()) {
			adtnlinfopage.clickViewSummary();
		} else {
	...
	public void validate_Services_when_edits_on_quantity() {
		...
		for (HashMap.Entry<String, String> entry : hashmap.entrySet()) {
			getDriver().findElement(By.xpath(entry.getKey())).clear();
			getDriver().findElement(By.xpath(entry.getKey())).sendKeys(entry.getValue());
		}
	...
	public void verifyDimension_alert_when_exceeded_dimension_updated_for_the_Booking() {
		...
		try {
			...
			getDriver().findElement(By.xpath("(//label[text()='Length (cm)']/following::input[1])[2]")).sendKeys("800",
					Keys.TAB);

			getDriver().findElement(By.xpath("(//label[text()='Width (cm)']/following::input[1])[2]")).sendKeys("900",
					Keys.TAB);
			getDriver().findElement(By.xpath("(//label[text()='Height (cm)']/following::input[1])[2]")).sendKeys("2100",
					Keys.TAB);
			getDriver().findElement(By.xpath("(//label[text()='Weight (kg)']/following::input[1])[2]")).sendKeys("60",
					Keys.TAB);
			uiTestHelper.scrolldown("300");
			consignmentInfoPage.clickConsignmentInfoNextBtn();
			uiTestHelper.scrolldown("300");
	...
	public void verifyIndividualpieceweight_alert_when_exceeded_weight_while_edit_the_Booking() {
		...
		try {
			...
			goodsInfoPage.clickGoodsInfoNextBtn();
			getDriver().findElement(By.xpath("(//label[text()='Weight (kg)']/following::input[1])[1]")).clear();
			getDriver().findElement(By.xpath("(//label[text()='Weight (kg)']/following::input[1])[1]")).sendKeys("71");
			consignmentInfoPage.clickConsignmentInfoNextBtn();
			uiTestHelper.scrolldown("300");
			addinfopage.clickValidServices();
			if (addinfopage.verifyGetPrice()) {
	...
	public void BK_Book_MultipleItems_totalWeight_exceeded() {
		...
		try {
			...
			getDriver().findElement(By.xpath("(//label[text()='Length (cm)']/following::input[1])[2]")).sendKeys("20",
					Keys.TAB);

			getDriver().findElement(By.xpath("(//label[text()='Width (cm)']/following::input[1])[2]")).sendKeys("20",
					Keys.TAB);
			getDriver().findElement(By.xpath("(//label[text()='Height (cm)']/following::input[1])[2]")).sendKeys("21",
					Keys.TAB);
			getDriver().findElement(By.xpath("(//label[text()='Weight (kg)']/following::input[1])[2]")).sendKeys("60",
					Keys.TAB);
